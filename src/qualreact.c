...
static void    tankmix5(Project *, int, double, double, double);
...
*/
{
    ...
    switch (net->Tank[i].MixModel)
    {
        ...
        case CSTR: tankmix5(pr, i, volin, massin, vnet); break;
    }
    return net->Tank[i].C;
}
...
void tankmix5(Project *pr, int i, double vin, double win, double vnet)
/*
**---------------------------------------------
**   Input:   i = tank index
**            vin = inflow volume
...
**            vnet = inflow - outflow
**   Output:  none
**   Purpose: updates quality in a CSTR tank model
**---------------------------------------------
*/
{
    Network *net = &pr->network;
    ...
    {
       cin = win/vin;
       if (vin > 0.0) seg->c = cin - (cin - seg->c) * exp(-(vin/seg->v) * dt);
       //seg->v += vnet;
       //seg->v = MAX(0.0, seg->v);
	   tank->C = seg->c;
    }
